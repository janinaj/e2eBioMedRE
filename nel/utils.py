...
def prepare_configs(args, verbose=True):
    if args.cg_config is None: 
        return None

    # Extract the requested config
    if verbose: 
        print('Config {}'.format(args.cg_config), flush=True)
    
    configs = pyhocon.ConfigFactory.parse_file(BASIC_CONF_PATH)[args.cg_config]

    configs['seed'] = args.seed
    configs['config_name'] = args.cg_config
    configs['dataset'] = args.dataset
    configs['task_learning_rate'] = args.task_learning_rate
    configs['cnn_text_depth'] = args.cnn_text_depth
    configs['feature_size'] = args.feature_size
    configs['cnn_text_dropout'] = args.cnn_text_dropout

    configs['do_predict_dev'] = args.do_predict_dev
    configs['do_predict_dev_gold'] = args.do_predict_dev_gold
    configs['do_predict_test'] = args.do_predict_test

    # save_dir
    hyperparams = "lr_{}-depth_{}-fs_{}-drop_{}".format(
        configs['task_learning_rate'], 
        configs['cnn_text_depth'],
        configs['feature_size'],
        configs['cnn_text_dropout']
    )

    configs['result_dir'] = join(
        BASE_RESULT_PATH, configs['dataset'], configs['config_name'], hyperparams
    ) 
    create_dir_if_not_exist(configs['result_dir'])
    configs['save_dir'] = join(
        BASE_SAVE_PATH, configs['dataset'], configs['config_name'], hyperparams
    )
    create_dir_if_not_exist(configs['save_dir'])

    # dir of fine-tuned model
    configs['finetuned_model_dir'] = join(
        configs['save_dir'], 'model.pt'
    )
