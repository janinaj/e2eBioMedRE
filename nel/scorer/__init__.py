...
def evaluate(model, dataset, ontology, configs, return_correct_cases=False, return_prediction=False):
    # Apply the model
    ...
    for inst in dataset.items:
        ...
        topk_entity_names = [n.name_str for n in inst.candidate_names]

        total_predictions.append(topk_entity_ids)
        total_predictions_names.append(topk_entity_names)
    ...
    if return_prediction:
        return eval_results, correct_cases, total_predictions, total_predictions_names
    print(f'model_inference_time = {model_inference_time}')
    return eval_results
